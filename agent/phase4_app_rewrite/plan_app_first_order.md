...
- arch-auditor（956889a 后）：六核查点全过（依赖矩阵/V26 单一所有者/§8.1/变量组隔离/§8.3/
...
### 36.4 完成记录（2026-07-23，代码本提交）

- 既有 `test_unknown_cmd_ignored` 用的正是 0x02（当时未处理）——修订后改用 0x03 保住
  「未知 cmd 丢弃」语义，另加 3 状态帧用例（原样透传/坏 len 丢弃/latest wins+独立 seq）。
- E01：vision 服务禁止前缀 0 命中；uart_vision include 面不变。
- E02：范围全在 §36.2（.ccsproject 会话前既存不纳入）。
- E03 主机 **584 PASS 0 FAIL**＝570 基线+14（driver 5 断言+service 9）。
- E04 固件 exit 0、0 诊断；vision.o 进 linkInfo（Vision_Update/SelectTopic 符号确证）。
- arch-auditor：六核查点全过、零阻断/零重要；gimbal 双消费风险核实为结构性不可达
  （Gimbal_Init 零调用者+GIMBAL_AIM 零进入点+单活动条目互斥）；1 建议级已采纳
  （Vision_Init 补 s_telemetry_on=false，与头注对称）。

## 37. PT3v 契约（param_tune schema v3：底盘/航向增益持久化 + CHAS/HEAD 组 + TurnTest 条目）——冻结

- **task_id**: PT3v-param-schema3
- **goal**: 关闭「上电就调」最后缺口：**底盘速度环增益目前不持久化**（每次上电必须
  重连 VOFA SpeedTune 重调，MS02 已登记），航向保持/定角转增益无任何调参入口。
  扩 param_tune blob schema 2→3：+底盘速度环增益（左右同值应用）+motion 航向调参
  （hold kp/ki/kd + turn_kp）+转弯测试角（自持，dist_mm 先例）；新增 CHAS/HEAD
  两参数组与 TurnTest 条目 idx11。
- **电赛四问**：验收动作=视觉行驶「4 圈误差不累计」的定角转与航向保持（0.25）+
  一切用 IMU 转弯的形态；基础线；标定入口=本任务交付本体。
- **拥有者扩面（Model A 无副本，两处小修订随本契约）**：
  ① chassis 增 `Chassis_GetSpeedGains(side, *kp,*ki,*kd)`（读回唯一所有者 Pid cfg，
  save 序列化用——S01 「已知空缺」口径的对称读口）；
  ② motion 增 `Motion_SetHeadingTuning(hold_kp,hold_ki,hold_kd,turn_kp)` +
  `Motion_GetHeadingTuning(4×out)`——写 s_cfg 四字段；**生效语义如实登记**：
  turn_kp 每拍活读=即时生效，hold 三增益在下次 Start*（Pid_Init 灌 cfg）生效；
  TurnTest on_enter 序（Init→ParamTune_Init→StartTurn）保证两者都生效。

### 37.1 增量接口与装配

```c
/* param_tune 增（milli 口径同现例；步长占位）：*/
ParamTune_Get/SetCKp_milli / CKi / CKd          /* 底盘速度环，应用=双轮同值 */
ParamTune_Get/SetHKp_milli / HKi / HKd / HTKp   /* motion 航向保持三增益 + 定角转 kp */
ParamTune_GetTurnDeg / SetTurnDeg               /* 转弯测试角（度，自持，默认 90） */
```

- blob schema 3 = v2 字段 + 8 新字段（旧 v2 记录长度不符→一次性失效退默认，v1→v2 先例）。
- 菜单：`CHAS`（C Kp/Ki/Kd + SAVE）、`HEAD`（H Kp/Ki/Kd、T Kp、Ang deg + SAVE）；
  任一 SAVE 仍序列化同一 blob 全量。
- `TurnTest` 条目 idx11：enter=Motion_Init(保守 cfg)→ParamTune_Init（重推持久增益）→
  `Motion_StartTurn(+TurnDeg)`；step=Motion_Update；exit=Motion_Stop。标定环=
  HEAD 组改增益→进页转一把→BACK→再改再进（servo_check 同款工作流）。
- ParamTune_Init 应用序扩展：现有 LF/剖面/距离 + Chassis_SetSpeedGains×2 +
  Motion_SetHeadingTuning。**底盘默认增益仍 0**（无实测依据不拍系数——上车 SpeedTune
  调出后经 CHAS 组录入 SAVE，此后每次上电即用）。

### 37.2 allowed_files

`hc-team/app/service/chassis/{chassis.h,chassis.c}`（+getter）；
`hc-team/app/service/motion/{motion.h,motion.c}`（+heading tuning setter/getter）；
`hc-team/app/service/param_tune/{param_tune.h,param_tune.c}`（schema v3）；
`hc-team/app/system/app_compose.c`（CHAS/HEAD 组+TurnTest idx11）；
`tests/host/{test_param_tune.c,test_chassis.c,test_motion.c}`（各追加）；本计划回写。
forbidden：`middleware/pid/**`、`Debug/makefile`（无新 .o）、其余一切。

### 37.3 证据行（4 行，恰 1 条固件构建行）

| 行 | 名称 | 命令 | 预期 |
|---|---|---|---|
| E01 | 依赖纯净 | param_tune include 面新增恰 chassis.h（Service 同层受控，line_follow/motion 先例同款）；arch-scan 空输出 | 如式 |
