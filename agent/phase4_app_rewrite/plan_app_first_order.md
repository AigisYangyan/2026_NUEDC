...
| Q11 | 整车几何标定：压角点转弯的「灰度阵列中心→驱动轴纵向距离 `L`」及车身长宽今天**零消费者**，按 S07 裁定属 T01 段表常量，**现在不建 struct 字段**。标定规格已出 `agent/GEOMETRY_CALIBRATION.md`：灰度间距/轮距/脉冲距映射既有单一所有者字段（`pitch_mm` track_error.h:33 / `track_width_mm` motion.h:58 / `mm_per_pulse` odometry.h:35），实测填值；`L` 定为**已有原语组合** `FOLLOW_UNTIL(角)→STRAIGHT(L)→TURN(90°)`（非新 motion 原语），与车身几何登记待 T01 段表定案（题目角点几何决定 `L` 微调）。**纠正**：高精度距离判断归 `mm_per_pulse` 非轮距。**云台几何裁定（2026-07-19，用户纠正）**：云台瞄准控制**单一所有者 = 上位机**，MSPM0 只执行/传话，**不做云台几何前馈**——撤回前一版 A5/A6/A7（云台→驱动轴/→灰度/横向偏移），几何杆臂前馈判为低收益、不建不测。运动时瞄准「防抖+精度」= 上位机环路整定；若需固件辅助仅「航向角速度反向补偿」（用现成 IMU 角速度，零几何常量，仍由上位机合成、单一所有者不变）。**待复核张力**：S05c 现状在 MSPM0 跑「瞄准收敛环」+ 驱动步进，与「控制定死上位机」有张力——须在 S05c 前馈接线/T01 前定死【甲】上位机给像素误差、MSPM0 收敛到步进（现状）还是【乙】上位机算好角度、MSPM0 纯执行；与 S05c 已「运动前馈推迟只留读点」一致，现不预支。 | S05c/T01 定甲乙 |
