...
## 0.2 契约修订：E03 模式自始不可满足（2026-07-17，本修订单独提交）

**修订前**：E03 用裸前缀 `IMU_UART_` 作为「旧器件驱动 API 已清除」的判据。

**为什么是错的**：`IMU_UART_` 同时命中**与旧器件无关**的既有符号 ——
`driver/board_uart/imu_uart.h` 的 include guard `IMU_UART_H`（3 处）与
`imu_uart.c` 的 `IMU_UART_RX_FIFO_SIZE` / `IMU_UART_TX_LOG_SIZE` 宏。
实证：`git show f0ef8f0:hc-team/driver/board_uart/imu_uart.h | grep IMU_UART_` 命中 3 行 ——
**该行在契约冻结的那一刻就已经不可能满足**，与本次施工无关。这是 DECIDE 阶段
未把模式先跑一遍现有代码树就冻结的结果。

**修订后**：模式改为逐一列出旧器件 API 的函数名前缀（`IMU_UART_Send` / `IMU_UART_RxByte` /
`IMU_UART_Process` / `IMU_UART_Get` / `IMU_UART_Calibration` / `IMU_UART_ResetUserData` /
`IMU_UART_RebootDevice` / `IMU_UART_WaitCalibration` / `IMU_UART_ClearAutoReport`），
其余分支不变。

**断言强度未被削弱**：E03 要证明的命题始终是「旧器件的 15 个公共 API 与其数据结构在全工程零残留」。
修订把量具从「会命中端口模块自有宏的裸前缀」换成「旧 API 函数名」，命题原样保留，
且修订后实测仍为 **0 命中** —— 不是靠放宽判据变绿的。

**这是同类错误第三次**（`plan_qei_gray_pinmux.md` §0.1 的 44/76 误报、`plan_p7` §0.1 的
`--warn_sections` 与 `test_signed_error_direction` 误报）。前两次的教训被记成「断言要锚定行首」，
但真正的根因更靠前：**证据行的模式在冻结前必须先对现有代码树跑一遍**，
否则冻结的可能是一条自始为假的行。

## 0.3 施工中查实：本机不能用文件名大小写证明删除

`git config core.ignorecase` 为 `true`。因此：

- `ls Debug/hc-team/driver/imu/IMU.o` 会**成功** —— 它解析到的是 `imu.o`。用 `ls` 判断
  「`IMU.o` 已不存在」会得到假阴性。
- `git rm IMU.c` 后再创建 `imu.c`，若中途执行 `git restore --staged IMU.c`，git 会把工作区的
  `imu.c` 与索引里的 `IMU.c` 视为**同一文件被修改**（` M IMU.c`），而非「删除 + 新增」。
  必须显式 `git rm --cached IMU.c IMU.h` 再 `git add imu.c imu.h`，索引才会记成 `D` + `A`。

**判据只能用 `git ls-files`**（走 git 索引，大小写敏感），不能用 `ls`。
与 `plan_p7` §0.1「`git show HEAD:` 不能判断工作区行尾」并列：**两者都是「工具答非所问」**。

## 0.4 本文件自身的编辑事故（2026-07-17）

插入 §0.1 时误把 `## 1. 器件事实` 标题整行替换掉，该缺陷已随修订提交 `27e50d7` 进入历史。
本次修订一并补回该标题。记录在案而非静默修复：`27e50d7` 的文档内容确实缺了一个标题。

## 1. 器件事实（来自 datasheet + 示例工程，全部已核对）
...
| E03 | `grep -rn "IMU_UART_Send\|IMU_UART_RxByte\|IMU_UART_Process\|IMU_UART_Get\|IMU_UART_Calibration\|IMU_UART_ResetUserData\|IMU_UART_RebootDevice\|IMU_UART_WaitCalibration\|IMU_UART_ClearAutoReport\|IMU_Update_Yaw\|IMU_Get_User_Yaw\|IMU_Get_Relative_Yaw\|Send_IMU_\|imu_measurement_t\|IMU_FUNC_" --include=*.c --include=*.h hc-team/` | — | **输出为空**；且 `hc-team/driver/imu/IMU.c`、`IMU.h` 施工前 `git ls-files` 命中 2、施工后命中 **0**（删除行按协议须证明「前有后无」；**须用 `git ls-files` 而非 `ls`**，见 §0.2）（**模式于 2026-07-17 修订，见 §0.2**） |
