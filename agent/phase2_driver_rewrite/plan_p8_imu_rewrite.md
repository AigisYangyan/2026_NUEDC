...
| E05 | `grep -n "UART_IMU_BAUD_RATE" Debug/ti_msp_dl_config.h` 与 `awk '/SYSCFG_DL_UART_IMU_init/,/^}/' Debug/ti_msp_dl_config.c \| grep -c "DL_UART_INTERRUPT_RX"` | — | BAUD 宏为 **115200**；`SYSCFG_DL_UART_IMU_init()` 体内 `DL_UART_INTERRUPT_RX` 命中 **1** —— 证明 RX 中断由 syscfg 单源开启，非手写 |
