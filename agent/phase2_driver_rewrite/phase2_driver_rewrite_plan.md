...
2026-07-16 流程自闭环（用户裁定，**权威版本，覆盖上一条及全部历史流程条款**）：不再有第二个施工者，REASONIX/GPT 承包商取消。现行流程为**单 agent 自闭环：决策 → 施工 → 验收**，协议见 `.agents/skills/embedded-closed-loop/SKILL.md`（三个 `reasonix-embedded-*` skill 已合并入此，`openai.yaml` 承包商注册与施工者 Prompt 已删除）。状态标签改为 `ACCEPTED`/`REJECTED`（`CODEX_*` 前缀退役；历史记录为当时事实，不回改）。

自闭环的代价与对策：三方流程里「验收者没写过这份代码」这一独立性是白送的，收成一个人后它消失了。唯一的机械替代品是——**任务契约（含全部 E 行）必须在写第一行生产代码之前提交进 git**，验收比对的是一份带时间戳、无法事后静默修改的契约，由 git 充当第二方。E 行中途发现写错是正常的，但必须**单独一次提交**说明改了什么、为什么，禁止与满足该行的代码同提交。先例：P6 的计划 §4 契约本身写错（返回类型误写 `void`），正是 `git show HEAD:<file>` 判定了责任在计划而非施工。

2026-07-16 硬件验收取消（用户裁定，覆盖本文件及五份计划中所有与之矛盾的条款）：**一切验收只做软件验收**——依赖扫描、主机测试、构建、diff 审读；软件行全过即标 `done`，验收结论只有 `CODEX_ACCEPTED`/`CODEX_REJECTED`。所有硬件验收行（原 P1F.T2、P2F.T2、P3 旧 E09–E12、P4.T3、P5 E14–E16）作废，板上调试与实测由用户自行负责。保留的底线：电机等功率器件的**软件侧安全设计**（上电安全态、单一所有者限幅、slew、换向过零死区、命令超时停止）仍是验收内容，用主机测试证明。§3 P0、§5 第 9 步、§7 硬件证据条、§8 硬件完成条按本裁定解读为“用户自理的操作性提醒”，不再是验收阻塞项。
