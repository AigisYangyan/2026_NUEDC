...
前置条件：P1F.T1 与 P4 均已验收合入（`mspm0_runtime.c` 无并行冲突）。（原 UART2 共享禁令因 `UART_IMU` 专用端口出现而作废，见修订 4。）  
...
- 主机测试框架 `tests/host/` 可复用；固件构建入口 `rtk make -C Debug all`；唯一硬件配置源 `project/mspm0/board.syscfg`。
