...
| D11 | imu（单轴 Z） | `driver/imu/` | `DONE` | **P8** | ⚠ **上位机配置 + 正方向实测**（见 §4.2） |
...
| 项 | 动作 |
|---|---|
| 输出速率 | 出厂 10 Hz → **改 200 Hz**（10 Hz 下 100 Hz 控制环 90% 的 tick 读到陈旧值，内环直接失效） |
| 波特率 | **保持 115200**（固件 syscfg 已配）。若改 230400 **必须告诉我**，我改 syscfg 一行 |
