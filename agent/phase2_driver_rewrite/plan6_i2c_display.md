...
**处置（2026-07-16，P6 之后单独提交）**：新表已入库（`e4ea890`），Q1–Q4 的依据可被他人复核，缺口关闭。

**后续（2026-07-16，用户裁定）**：旧表 `docs/主控板引脚表_G3519.xlsx` 的删除已确认并提交；曾一并入库的可读导出 `docs/pin_table_v2/` 亦按「docs/ 只保留给硬件组的最新文件」裁定删除；`MIGRATION_G3507_TO_G3519.md` 移入 `agent/`。**本条已闭环。**
