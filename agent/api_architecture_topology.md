...
最后复核：2026-07-17（S04：App Service 层第四个模块 `hmi` 落地，人机输入/显示服务收口 §5.3 补注、V14 提供替代路径；S03：App Service 层第三个模块 `tuning`+`tuning_chassis` 落地，VOFA 调参链路收口 §5.5 新增，V19 关闭；S02：`line_follow`+`lost_line` 落地，多环级联接线 `line_follow → chassis`，§5.4 灰度→循迹误差链从 planned 转为已接线首个真实消费者；S01：`chassis` 落地，§15 上层重置裁定解除）  
...
| V14 | UI 直接调用 Key/OLED Driver，并在 UI 头暴露 Key 类型 —— **S04 已提供替代路径**（2026-07-17）：新 `app/service/hmi` 以合法 Service→Driver 依赖复刻按键语义映射+行式显示能力，K1..K4→UP/DOWN/ENTER/BACK 唯一映射点下沉；旧 `menu_core.*`/`menu_pages.c` 冻结存量债待 T01 整体删除，本条本身保持 open（旧代码未删，违规仍在） | `menu_core.*`、`menu_pages.c`（旧）；替代路径 `hc-team/app/service/hmi/hmi.c/.h`（新） | UI 应通过 App 接口/Service，不直接操作 Driver | App Service/UI 阶段 → T01 关闭 |
