...
最后复核：2026-07-17（S02：App Service 层第二个模块 `line_follow`+`lost_line` 落地，多环级联接线 `line_follow → chassis`，§5.4 灰度→循迹误差链从 planned 转为已接线首个真实消费者；S01：`chassis` 落地，§15 上层重置裁定解除）  
...
| V10 | Service 目录当前没有有效源 API **partially closed 2026-07-17（S01→S02）**：`app/service/chassis`（S01）与 `app/service/line_follow`+`lost_line`（S02）落地，Service→Driver/Middleware/同层 Service 合法业务桥两例；`app/service/` 内除 chassis/line_follow 外仍无源 API，其余规划中的 Service（云台/VOFA 等）未开工，本条继续 open 直至全部补齐 | `hc-team/app/service/chassis/chassis.c/.h`、`hc-team/app/service/line_follow/line_follow.c/.h`+`lost_line.c/.h` | 缺少 Driver 与 Middleware 的业务桥 | Phase4 S03→S04 逐项补齐 |
...
| App Service | 其余规划层 | `app/service/`（除 chassis/line_follow 外）当前无有效 `.c/.h` API，S03→S04 待补 |
