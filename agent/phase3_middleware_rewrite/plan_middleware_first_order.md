...
| # | 模块 | 目录 | 状态 | 派工 | 说明 |
|---|---|---|---|---|---|
| M01 | pid | `middleware/pid/` | `DONE` | 契约 `0062332` / 代码 `3ab13fe` | E01–E05 全兑现（117 PASS/0 FAIL，固件 0 诊断）；V13 PID 部分达成关闭条件；评审 findings 处置见 §6 |
| M02 | 循迹误差估计器 | `middleware/track_error/` | `DONE` | 契约 `77e5926`（修订 `76656dd`/`100ecdb`）/ 代码 `f07601d` | 加权重心；E01–E04 全兑现（128 PASS/0 FAIL，固件 0 诊断）；`pitch_mm`/`bit0_is_left` 为配置项（位序唯一修正点，待 H2 实测定值）；评审 findings 处置见 §5.2 |
...
## 5. M01 评审 findings 处置（arch-auditor，2026-07-17）

阻断/重要级：**无**。建议级 2 条，均判「存量债，随上层重置处置，本轮不动」：

1. **`task_groups.c` `Task_MotorPidControl` 成为增益恒零的死代码**——无任务组引用、
   `g_vofa_cmd_lm/rm` 无写入者；旧实现靠共享全局「借」speed_loop 增益的意外通道被 M01
   正确切断。处置：登记进上层重置删除清单（连同 `g_vofa_cmd_*`），重置时不得作为参照复制。
2. **`out_limit = 1000.0f` 在 speed_loop/task1/task_groups 三处硬编码**，与
   `driver/motor/motor.h` 的 `MOTOR_OUTPUT_MAX 1000` 隐式耦合。处置：上层重置建 Service 时
   统一改用 `(float)MOTOR_OUTPUT_MAX`（Service 可见 Driver 头，合法）；存量债本轮不动。

附注（语义变化，方向正确）：task1 与 speed_loop 的 VOFA 调参不再经共享全局互相渗透——
这正是拆除可写全局的目标效果。

### 5.2 M02 评审 findings 处置（arch-auditor，2026-07-17）

阻断/重要级：**无**。建议级 1 条：中心常数 5.5f 手写字面量与路数常数重复编码同一事实
——**当场已修**（改为由 `TRACK_ERROR_CHANNEL_COUNT` 推导，测试零回归，随收工提交入库）。

移交备忘（auditor 核实的事实，上层重置时用）：旧 `track_follow.c:120` 与 M02 **不是等价替换**
——旧版 int16 ±55 限幅 + 丢线按 `last_error` 方向回退 ±27（带记忆）；新版 float mm、丢线
返回 false 无记忆。回退/记忆策略按契约归未来 Service，移交时须在 Service 里显式重建该策略，
不得以「换个函数名」方式迁移。

## 6. M02 契约 —— 循迹误差估计器（加权重心）

Status: pending
Goal: 新建 `middleware/track_error/`，把 12 路灰度深色位图量化为带符号横向误差（加权重心，
物理单位 mm），成为位序左右修正与误差量化的唯一所有者（生效于上层重置移交后）；
主机测试与固件构建双绿，App 层零调用（预期状态）。

### 决策依据（2026-07-17 用户拍板 + 手册查证）

- 用户：**加权重心**；白底黑线（黑=置位，与 `Gray_ReadDarkBitmap()` 的 dark 语义直接对齐）；
  间距读手册；居中加权、居中所限。
- 手册查证（《迹系列 20230119》，PyMuPDF 提文）：**间距不是器件常数**——迹系列是独立探头
  （宽 9.9mm，p9），间距/分布由用户机械安装自定义（p12–13）→ **间距必须是配置项**，
  固件无默认值可辩护；10mm 线宽下最多 2 路同时压线、不丢线至少 1 路（p38）；
  厂商 bit 序约定 P1=最右（p34）与本仓 syscfg 注释矛盾（gray.h 位序警告）→
  **bit0 在车左还是车右做成配置**，等 H2 实测定值。
- 居中加权：探头坐标 `(index − 5.5) × pitch_mm` 关于中心对称；加权重心 = 置位通道坐标平均。
  |误差| ≤ 5.5×pitch 天然成立（居中所限），无需额外限幅所有者。

### Architecture

- Abstraction：无状态纯函数——深色位图 → 加权重心横向误差。
  仅一个入口 `TrackError_FromDarkBitmap()`；有线返回 true 并输出误差，
  丢线（低 12 位全 0）返回 false 且不写输出。**不再多一个接口。**
- Hidden state：无（无状态）。丢线回退/记忆是控制策略，归未来 Service；
  全黑（十字/停止线）重心≈0 是固有性质，特征识别归调用者——均不进本模块。
- Owner layer：Middleware。不含 `driver/gray/gray.h`（依赖矩阵 Middleware→Driver 禁止），
  自持 `TRACK_ERROR_CHANNEL_COUNT 12`；与 Driver 路数的一致性由未来 Service 负责。
- 符号约定：一切左右=驾驶员视角；**+误差 = 线在车中心右侧**。
- ⚠ 过渡态双实现登记：存量债 `app/tasks/track_follow/Calculate_Track_Error()`（V03/V13）
  继续存在但**本契约禁止触碰**；所有权在上层重置删除 track_follow.c 时移交 M02
  （同 D12 灰度 Driver 先例）。此为已登记过渡，不是新增 §8.2 违规。
...
typedef struct {
    float pitch_mm;      /* 相邻探头中心间距，>0；机械安装事实（手册 p12-13，无默认值） */
    bool  bit0_is_left;  /* bit0(=PIN_IN1) 是否位于车左（驾驶员视角）。
                            左右位序的唯一修正点（gray.h 位序警告的落点），等 H2 实测定值 */
} TrackError_Config_T;

/* 加权重心量化。dark_bitmap 仅低 12 位有效（高位内部屏蔽）。
 * 有线：返回 true，*out_error_mm = 带符号误差（+ = 线在车中心右侧）。
 * 丢线：返回 false，不写 *out_error_mm。 */
...
- hc-team/middleware/track_error/track_error.h（新建）
- hc-team/middleware/track_error/track_error.c（新建）
- tests/host/test_track_error.c（新建）
- tests/host/Makefile
- Debug/makefile
- Debug/sources.mk
- Debug/ccsObjs.opt
...
- hc-team/app/tasks/track_follow/track_follow.c（V03 存量债，移交前不动）
...
1. 既有 117 个主机用例全过，M01 成果零回归。
2. track_follow.c / task1 现行循迹链路为零改动（M02 零调用者=预期，禁止制造调用者）。
3. Driver gray 的原子读取与「不声明左右」性质不动。

### Steps

1. 先写 test_track_error.c（新符号未实现→编译红=失败复现）。
2. 最小实现 track_error.h/.c；Debug 构建接线（subdir_*.mk + makefile + sources.mk + ccsObjs.opt）。
3. 复现 E01–E04；arch-auditor；topo-updater 收工（登记新模块与过渡态双实现）。

### Verification（证据行，冻结）

- E01 command: Grep `#include\s+"(driver/|app/|ti_msp)|DL_[A-Z]|NVIC_|GPIO_|IRQ|__disable_irq` in `hc-team/middleware/`
...
- E03 command: PowerShell `rtk proxy make -C tests/host all`（clean 后全量）
- E03 expected_exit: 0
- E03 postcondition: 0 FAIL；test_track_error 用例 ≥9 全 PASS；全套 PASS 总数 ≥126（基线 117 + ≥9）

- E04 command: PowerShell `rtk make -C Debug all`
- E04 expected_exit: 0
- E04 postcondition: 0 error / 0 warning；`track_error.o` 出现在编译与链接产物中，.out/.hex 正常（唯一固件构建行）

### Stop conditions

- 发现 Debug 构建体系无法以手工 mk 文件接新目录（需 CCS 重生成）→ 停，报告替代方案。
- 任何一步需要改 track_follow.c / gray.* 才能继续 → 停（所有权移交只发生在上层重置）。
- 测试中发现加权重心在真实位图样例下需要额外滤波/记忆才能自洽 → 停，扩展契约单独提交。
