...
| **网表对照 + QEI 左右命名对调（2026-07-17）** | Driver 封包后硬件组交来拓展板网表，逐网 ↔ syscfg 全量交叉：灰度连接器 1–12 与 IN1–IN12 直通零交叉、H1 坐实 5V 供电（定案：保持 5V + 12 路串阻）、发现 U33/U34 插座电机-编码器左右交叉（M1）。用户裁定「硬件改动最小化，固件能改名吸收的固件吸收」：定义 **U33=左轮、U34=右轮**（驾驶员视角），`board.syscfg` 对调 QEI `$name` —— 现 **`QEI_LEFT`=TIMG8/PB10/PB11(U33)、`QEI_RIGHT`=TIMG9/PB7/PB9(U34)**（覆盖下方 07-16 行的归属，历史行不改写）。另发现 K4 断线/PA0-PA1 上拉/IMU TX 电平三项移交硬件组 | 契约冻结 `6943172`，定稿 **`docs/给硬件组的修改方案.md`**、`docs/网表对照结论.md`；主机 109 项、构建 0 诊断、驱动代码零改动 |
