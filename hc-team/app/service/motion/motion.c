...
/* PROFILED_STRAIGHT 防跑飞看门狗：已运行拍数（每次 motion_step_profiled_straight 自增）。 */
...
static void motion_step_profiled_straight(bool heading_valid)
{
    ...
    /* §8.1 防跑飞看门狗：运行超过拍数上限（如编码器脱线致 dist 永不达标）→ 确定性停止。
     * 完成判据优先；本条是安全兜底，所有者=motion（不外泄，超时值由 cfg 标定；0=禁用）。 */
